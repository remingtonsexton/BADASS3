...
import bifrost
import spectres
import badass_tools.gh_alternative as gh_alt # Gauss-Hermite alternative line profiles
...
# TODO: don't need to import common template utils once all templates have been classify transitioned
...
class BadassRunner:
    ...
    def run(self):
        ...
        for i, target in enumerate(self.targets):
            ...
            ctx = BadassContext(target, topts)
            if USE_MULTIPROCESS:
                ctx.start()
                jobs.append(ctx)
            else:
...
class BadassContext(mp.Process):
    ...
    def run(self):
        ...
        mask_emline         = self.options.fit_options.mask_emline
        mask_metal          = self.options.fit_options.mask_metal
        fit_stat            = self.options.fit_options.fit_stat
        n_basinhop          = self.options.fit_options.n_basinhop
        test_outflows       = self.options.fit_options.test_outflows
        test_line           = self.options.fit_options.test_line
        max_like_niter      = self.options.fit_options.max_like_niter
        output_pars         = self.options.fit_options.output_pars
        cosmology           = self.options.fit_options.cosmology
        mcmc_fit            = self.options.mcmc_options.mcmc_fit
        nwalkers            = self.options.mcmc_options.nwalkers
        auto_stop           = self.options.mcmc_options.auto_stop
        conv_type           = self.options.mcmc_options.conv_type
        min_samp            = self.options.mcmc_options.min_samp
        ncor_times          = self.options.mcmc_options.ncor_times
        autocorr_tol        = self.options.mcmc_options.autocorr_tol
        ...
        fit_absorp          = self.options.comp_options.fit_absorp
        tie_line_disp       = self.options.comp_options.tie_line_disp
        tie_line_voff       = self.options.comp_options.tie_line_voff
        n_moments           = self.options.comp_options.n_moments
        plot_param_hist     = self.options.plot_options.plot_param_hist
    ...
    def pca_reconstruction(self):
        ...
        if not self.options.pca_options.do_pca:
            if (np.isnan(self.spec).any()) or (np.isnan(self.noise).any()):
                raise ValueError("The flux or flux error in fitting region ({mi}, {ma}) is nan, stopping fit. Change fitting region or enable PCA to cover nan region.".format(mi=self.fit_reg.min, ma=self.fit_reg.max))
            self.log.pca_information()
            return

        pca_masks = self.options.pca_options.pca_masks
        self.log.info(f"Performing PCA on a spectrum with nans over region(s) {pca_masks}. Be careful to ensure PCA covers all nan regions, else PCA will fail.")
        self.log.info(" Performing PCA analysis...\n")
        if len(pca_masks):
            pca_reg_test = [(i[0]>=self.fit_reg.min,i[1]<=self.fit_reg.max) for i in pca_masks] # check that pca mask regions are within fitting region
            if not np.all(pca_reg_test):
                raise ValueError("PCA region masks {pca_masks} must be within fitting region ({mi}, {ma})".format(pca_masks=pca_masks, mi=self.fit_reg.min, ma=self.fit_reg.max))
        else:
            pca_masks = ([(self.fit_reg.min, self.fit_reg.max)])
            self.options.pca_options.pca_masks = pca_masks

        wave_input = np.array(self.wave)
        flux_input = np.array(self.spec)
        err_input = np.array(self.noise)
        ...
        if flux_nan_check:
            self.log.debug('\tnans detected in spectrum flux. Setting to spectrum mean and performing PCA.')
        ...
        if err_nan_check:
            self.log.debug('\tnans detected in spectrum flux error. Setting to 0.1*flux at corresponding wavelength.')
        ...
        if isinstance(n_components, int):
            ...
            self.log.warning(f"\tWarning: {n_components} is invalid argument for number of PCA components. Must be int or None. Defaulting to 20 components.")
        ...
        if self.options.plot_options.plot_pca:
            ...
            plt.tight_layout()
            plt.savefig(self.outdir.joinpath('pca_spectrum.pdf'))
            plt.style.use('dark_background')
            plt.close(fig)

        self.log.info(" PCA analysis complete!")
        self.log.info("---------------------------------------")
        self.log.pca_information(pca_nan_fix=True, pca_exp_var=evals_cs[-1])
...
# TODO: classify and use
def metal_masker_bifrost(wave,spec,noise,fits_file):
    """
...
# TODO: add to user input class and/or prepare_context
