...
"""Bayesian AGN Decomposition Analysis for SDSS Spectra (BADASS2) Python 3 Version
...
#   analysis for paramter chain convergence, which now produces the most robust estimates.
...
#   better fit, namely for the power-law slope paramter which never varies with 
...
# - Bug fixes; fixed problems with paramters in 'list' option conv_type getting
#   removed.  Now if a user-defined paramter in conv_type is wrong or removed,
...
# Version 7.6.0 - 7.6.1
# - Last Python 2 update before major transition to Python 3
...
def check_args(file,run_dir,temp_dir,
			   fit_reg, good_thresh,interp_bad,
			   test_outflows, outflow_test_niter,
			   max_like_niter, min_sn_losvd,
			   mcmc_fit, nwalkers, auto_stop, conv_type, 
			   min_samp, ncor_times, autocorr_tol,
			   write_iter, write_thresh, burn_in, 
			   min_iter,max_iter,
			   fit_feii, fit_losvd, fit_host, fit_power,
			   fit_broad, fit_narrow, fit_outflows, tie_narrow,
			   plot_param_hist, plot_flux_hist, plot_lum_hist,
			   plot_mbh_hist, plot_corner,plot_bpt,write_chains,print_output,
			   threads):
	"""
	This function checks argumnets passed to run_BADASS() function to ensure
	correct types and input format.  These will raise an error and stop the 
	code from proceeding.
	"""
	# file, run_dir, temp_dir
	d =  {'file':file,'run_dir':run_dir,'temp_dir':temp_dir}
	# for i,key in enumerate(d):
	for key,value in sorted(d.items()):
		if (not isinstance(d[key],str)) or (not os.path.exists(d[key])):
			raise TypeError('\n\n	 Input Error: %s is not a valid path of type(str) for %s keyword! \n' % (d[key],key))
	# fit_reg = (min,max), min<max, min>3460 , max< 9464(Indo-Us Coude Feed Stellar Spectra Library)
	if not isinstance(fit_reg,(list,tuple)): 
			if fit_reg=='auto':
				pass
			else:
				raise TypeError('\n\n fit_reg must be a list or tuple of format (min,max) in Angstroms! \n ')
	elif (fit_reg[0]>fit_reg[1]): 
		raise ValueError('\n\n	 Input Error: fit_reg min cannot be greater than max! \n ')
	elif (fit_reg[0]<3460 or fit_reg[1]>9464) and (fit_losvd==True): 
		raise ValueError('\n\n	 Input Error: The fit_reg (min,max) for the Indo-US Stellar Templates is (3460,9464)! \n') 
	# good_thresh [0.0,1.0]
	if (good_thresh < 0.0) or (good_thresh > 1.0): 
		raise ValueError('\n\n	 Input Error: good_thresh must be a value in range [0.0,1.0]! \n')
	# outflow_test_niter,min_sn_losvd,write_iter,write_thresh,burn_in,min_iter,max_iter,min_samp > 0
	d = {'outflow_test_niter':outflow_test_niter,'write_iter':write_iter,'write_thresh':write_thresh,
		 'burn_in':burn_in,'min_iter':min_iter,'max_iter':max_iter,'min_samp':min_samp}
	for key,value in sorted(d.items()):
		if (not isinstance(d[key],int)) or (d[key]<0.0):
			raise ValueError('\n\n	 Input Error: %s must be of type(int) and > 0.0! \n' % (key))
	# min_sn_losvd > 0.0
	if (min_sn_losvd < 0.0):
		raise ValueError('\n\n	 Input Error: min_sn_losvd > 0.0! \n ')
	# max_like_niter > 1
	if (max_like_niter < 1.0):
		raise ValueError('\n\n	 Input Error: max_like_iter > 1.0.  You must do at least one max. likelihood fit to get initial parameter values! \n ')
	# min_samp > write_iter
	if (min_samp < write_iter):
		raise ValueError('\n\n	 Input Error: You really should perform at least as many sampling iterations as writing iterations! \n ')
	#  max_iter > min_iter
	if (max_iter < min_iter):
		raise ValueError('\n\n	 Input Error: your maximum number of iterations should be more than your minimum! \n ')
	if (min_iter<write_iter) or (min_iter<write_thresh):
		raise ValueError('\n\n	 Input Error: your minimum number of iterations should be at least as much as your write_iter and write_thresh! \n ')
	# theads > 0 
	if (threads < 0) or not isinstance(threads,int):
		raise ValueError('\n\n	 Input Error: threads should be a type(int) and > 0 \n ')
	# Boolean keywords
	d = {'interp_bad':interp_bad,'test_outflows':test_outflows,'mcmc_fit':mcmc_fit,'auto_stop':auto_stop,'fit_feii':fit_feii,'fit_losvd':fit_losvd,
		 'fit_host':fit_host,'fit_power':fit_power,'fit_broad':fit_broad,'fit_narrow':fit_narrow,
		 'fit_outflows':fit_outflows,'tie_narrow':tie_narrow,'plot_param_hist':plot_param_hist,
		 'plot_flux_hist':plot_flux_hist,'plot_lum_hist':plot_lum_hist,'plot_mbh_hist':plot_mbh_hist,
		 'plot_corner':plot_corner,'plot_bpt':plot_bpt,'write_chains':write_chains,'print_output':print_output}
	for key,value in sorted(d.items()):
		if (not isinstance(d[key],(bool,int))):
			raise TypeError('\n\n	 Input Error: %s must be of type(bool)! \n' % (key))
...
def run_BADASS(file,run_dir,temp_dir,
			   fit_reg=(4400,5800), good_thresh=0.60,interp_bad=False,
			   test_outflows=True, outflow_test_niter=10,
			   max_like_niter=10, min_sn_losvd=2,
			   mcmc_fit=True, nwalkers=100, auto_stop=True, conv_type='median', 
			   min_samp=2500, ncor_times=2.0, autocorr_tol=5.0,
			   write_iter=100, write_thresh=100, burn_in=47500, 
			   min_iter=100,max_iter=50000,
			   ...
			   outflow_test_pars={'amp_test':{'test':True,'nsigma':3.0},
			   					  'fwhm_test':{'test':True,'nsigma':1.0},
			   					  'voff_test':{'test':False,'nsigma':1.0},
			   					  'rov_test':{'test':True,'nsigma':2.0},
			   					  'bounds_test':{'test':True,'nsigma':1.0}},
			   plot_param_hist=True, plot_flux_hist=True, plot_lum_hist=True,
			   plot_mbh_hist=True, plot_corner=False,plot_bpt=True,write_chains=False,print_output=True,
			   threads=4,
			   ):
	"""
	This function calls all other sub-functions in order. 
	"""
	# This is the main function responsible for executing the fitting steps in order
	# by calling all functions below.
	check_args(file,run_dir,temp_dir,
			   fit_reg, good_thresh,interp_bad,
			   test_outflows, outflow_test_niter,
			   max_like_niter, min_sn_losvd,
			   mcmc_fit, nwalkers, auto_stop, conv_type, 
	...
	if (test_outflows==True) & (fit_outflows==True) & (outflow_test_niter>0) & (fit_reg[1]>4600.): # 
		if print_output:
			...
			print('----------------------------------------------------------------------------------------------------')
		if ( (fit_reg[0]<=4400.) & (fit_reg[1] >=5800.) ): # if Hb/[OIII] region available
			if print_output:
				print(' Testing the Hb/[OIII] region...')
			fit_outflows,outflow_res,res_sigma = outflow_test_oiii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output)
			...
			fit_outflows,outflow_res,res_sigma = outflow_test_nii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output)
			if fit_outflows==True:
				if print_output:
					print('\n Outflows detected: including outflow components in fit.')
			elif fit_outflows==False:
	...
	if print_output:
		...
		print('----------------------------------------------------------------------------------------------------')
	param_dict = initialize_mcmc(lam_gal,galaxy,fwhm_gal,velscale,run_dir,fit_reg=fit_reg,fit_type='init',
	...
	result_dict, comp_dict, sn = max_likelihood(param_dict,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
									 temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
									 test_outflows=False,outflow_test_niter=outflow_test_niter,max_like_niter=max_like_niter,
	...
	if print_output:
		...
		print('----------------------------------------------------------------------------------------------------')
	param_dict = initialize_mcmc(lam_gal,galaxy,fwhm_gal,velscale,run_dir,fit_reg=fit_reg,fit_type='final',
								 fit_feii=fit_feii,fit_losvd=fit_losvd,fit_host=fit_host,
								 fit_power=fit_power,fit_broad=fit_broad,
...
def estimate_BH_mass_hbeta(fwhm,fwhm_low,fwhm_upp,L5100,run_dir,n_samp=1000,plot_mbh_hist=True):
	...
	B_ = np.random.choice(a=x_B,size=n_samp,p=p_B,replace=True)
	#
	L5100   = L5100[(L5100/L5100 == 1) & (L5100 >= 0.0)]
	FWHM_Hb = FWHM_Hb[(L5100/L5100 == 1) & (L5100 >= 0.0)]
...
def estimate_BH_mass_halpha(fwhm,fwhm_low,fwhm_upp,L5100,run_dir,n_samp=1000,plot_mbh_hist=True):
	...
	#
	L5100 = L5100[(L5100/L5100 == 1) & (L5100 >= 0.0)]
	FWHM_Ha = FWHM_Ha[(L5100/L5100 == 1) & (L5100 >= 0.0)]
...
def initialize_mcmc(lam_gal,galaxy,fwhm_gal,velscale,run_dir,fit_reg,fit_type='init',fit_feii=True,fit_losvd=True,fit_host=True,
					fit_power=True,fit_broad=True,fit_narrow=True,fit_outflows=True,tie_narrow=True,print_output=True):
	"""
	Initializes all free paramters to be fit based on the fitting region and estimate some reasonable
	...
	# However, this usually leads to unrealistic fitting of noise spikes.  The typical uncertainty in stellar velocity dispersion
	# is ~15 km/s. We set this as the upper limit for stellar dispersion, and set a min_fwhm = 2.355*15 = 35 km/s for narrow lines, 
	# and 500 km/s for broad lines.
	...
	min_fwhm = 35.0 # km/s; typical uncertainty 
...
def rov_test(resid_outflow,resid_no_outflow,run_dir):
	"""
	Ratio of Variances test:
	The ratio of the variance of the residuals of the no-outflow model
	and the variance of the residuals of outflow model for each iteration
	of the outflow test. 
	"""

	# For multiple runs
	rov_arr = np.empty(np.shape(resid_outflow)[0])

	for i in range(np.shape(resid_outflow)[0]):
		# Compute median and std of residual standard deviations
		std_resid_outflow        = np.std(resid_outflow[i,:])
		std_resid_no_outflow     = np.std(resid_no_outflow[i,:])
		rov = (std_resid_no_outflow)**2/(std_resid_outflow)**2 # ratio of variances
		rov_arr[i] = rov

	return np.median(rov_arr), np.std(rov_arr)

def outflow_test_oiii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	"""
	...
	param_dict_outflows = initialize_mcmc(lam_gal,galaxy,fwhm_gal,velscale,run_dir,fit_reg=fit_reg,fit_type='init',
	...
	mcpars_outflow, mccomps_outflow = max_likelihood(param_dict_outflows,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
										None,None,None,fwhm_gal,velscale,None,None,run_dir,test_outflows=True,outflow_test_niter=outflow_test_niter,
										print_output=print_output)
	# Perform fitting with NO outflows
	if print_output:
		print('\n Fitting without outflow components...')
	param_dict_no_outflows = initialize_mcmc(lam_gal,galaxy,fwhm_gal,velscale,run_dir,fit_reg=fit_reg,fit_type='init',
								 fit_feii=True,fit_losvd=True,
								 fit_power=True,fit_broad=True,
								 fit_narrow=True,fit_outflows=False,tie_narrow=False,print_output=print_output)

	mcpars_no_outflow, mccomps_no_outflow = max_likelihood(param_dict_no_outflows,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
										None,None,None,fwhm_gal,velscale,None,None,run_dir,test_outflows=True,outflow_test_niter=outflow_test_niter,
										print_output=print_output)


	# Determine if there is a significant improvement in residuals to warrant inclusion of a second component
	# From the outflow fit paramters, get the median values for the widths and velocity offsets to determine th 
	...
	# Calculate ratio-of-variances (rov)a and its uncertainty
	rov, rov_err = rov_test(resid_outflow,resid_no_outflow,run_dir)
	...
	for key in mcpars_outflow:
		...
		param_flags = 0
		if ( (mc_med-mc_std*outflow_test_pars['bounds_test']['nsigma']) <= param_dict_outflows[key]['plim'][0]):
			param_flags += 1
		if ( (mc_med+mc_std*outflow_test_pars['bounds_test']['nsigma']) >= param_dict_outflows[key]['plim'][1]):
			param_flags += 1
	...
	for key in mcpars_no_outflow:
		...
		param_flags = 0
		if ( (mc_med-mc_std*outflow_test_pars['bounds_test']['nsigma']) <= param_dict_no_outflows[key]['plim'][0]):
			param_flags += 1
		if ( (mc_med+mc_std*outflow_test_pars['bounds_test']['nsigma']) >= param_dict_no_outflows[key]['plim'][1]):
			param_flags += 1
	...
	if print_output:  
		...
		print('{0:<30}{1:<25.2f}{2:<25.2f}'.format('ratio-of-variances',rov,rov_err))
	...
	write_log((pdict,
			   ...
			   amp_metric,fwhm_metric,voff_metric,rov,rov_err,\
	...
	if (outflow_test_pars['amp_test']['test']==True):
		#	(1) Amp. test: (AMP_outflow - dAMP_outflow) > sigma
		amp_cond = (amp_metric > (outflow_test_pars['amp_test']['nsigma']) )
		if (amp_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['amp_test']['pass'] = True # Add to outflow_test_pars dictionary 
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow amplitude condition:', 'Pass' ) )
		elif (amp_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['amp_test']['pass'] = False # Add to outflow_test_pars dictionary 
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow amplitude condition:', 'Fail' ) )

	#	(2) FWHM test: (FWHM_outflow - dFWHM_outflow) > (FWHM_core + dFWHM_core)
	if (outflow_test_pars['fwhm_test']['test']==True):
		fwhm_cond = ( fwhm_metric > (outflow_test_pars['fwhm_test']['nsigma']) )
		if (fwhm_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['fwhm_test']['pass'] = True # Add to outflow_test_pars dictionary 
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow FWHM condition:'	 , 'Pass') )
		elif (fwhm_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['fwhm_test']['pass'] = False # Add to outflow_test_pars dictionary 
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow FWHM condition:'	 , 'Fail') )

	#	(3) VOFF test: (VOFF_outflow + dVOFF_outflow) < (VOFF_core - dVOFF_core)
	if (outflow_test_pars['voff_test']['test']==True):
		voff_cond = (voff_metric>(outflow_test_pars['voff_test']['nsigma']))
		if (voff_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['voff_test']['pass'] = True
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow VOFF condition:'	 , 'Pass ') )
		elif (voff_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['voff_test']['pass'] = False
			if print_output:
				print('{0:<35}{1:<30}'.format('Outflow VOFF condition:'	 , 'Fail' ) )

	#   (4) Ratio of variances:
	if (outflow_test_pars['rov_test']['test']==True):
		rov_cond = ( rov > (outflow_test_pars['rov_test']['nsigma']))
		if (rov_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['rov_test']['pass'] = True
			if print_output:
				print('{0:<35}{1:<30}'.format('ratio-of-variances condition:', 'Pass' ) )
		elif (rov_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['rov_test']['pass'] = False
			if print_output:
				print('{0:<35}{1:<30}'.format('ratio-of-variances condition:', 'Fail' ) )

	#	(5) Parameter bounds: if paramter reached the parameter limits (bounds), then its uncertainty is zero, and something went wrong with the fit.
	if (outflow_test_pars['bounds_test']['test']==True):
		...
		if (bounds_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['bounds_test']['pass'] = True
			if print_output:
				print('{0:<35}{1:<30}'.format('Parameter bounds condition:', 'Pass' ) )
		elif (bounds_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['bounds_test']['pass'] = False
			if print_output:
	...
	output_model = False
	comp_dict_outflow = fit_model(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
						  None,None,None,fwhm_gal,velscale,None,None,run_dir,
	...
	fit_type	 = 'outflow_test'
	output_model = False
	comp_dict_no_outflow = fit_model(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
						  None,None,None,fwhm_gal,velscale,None,None,run_dir,
						  fit_type,output_model)
	...
	if not outflow_conds: # outflow_conds is empty because no tests were performed but we tested for outflows anyway
		...
		# Write to log
		write_log(outflow_test_pars,'outflow_test_pass',run_dir)
		return True,pdict,sigma
	elif (all(outflow_conds)==False):
		# Write to log
		write_log(outflow_test_pars,'outflow_test_fail',run_dir)
...
def outflow_test_plot_oiii(comp_dict_outflow,comp_dict_no_outflow,run_dir):
	...
	ax2.plot(comp_dict_no_outflow['wave']['comp'], 3*1*comp_dict_no_outflow['noise']['comp'], color='xkcd:dodger blue' , linewidth=0.5, linestyle='--')
	...
	ax3.plot(comp_dict_outflow['wave']['comp'], comp_dict_outflow['na_feii_template']['comp']   , color='xkcd:yellow'     , linewidth=1.0, linestyle='-' , label='Na. FeII'     )
	ax3.plot(comp_dict_outflow['wave']['comp'], comp_dict_outflow['br_feii_template']['comp']   , color='xkcd:orange'     , linewidth=1.0, linestyle='-' , label='Br. FeII'     )
	...
	ax4.plot(comp_dict_outflow['wave']['comp'], 3*1*comp_dict_outflow['noise']['comp'], color='xkcd:dodger blue' , linewidth=0.5, linestyle='--')
...
def outflow_test_nii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	"""
	Performs outflow tests using the outflow criteria on the [OIII] outflows.
	"""
	# The optimal fitting region for fitting outflows in [OIII] is (4400,5800) with ALL comoponents
...
def max_likelihood(param_dict,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
				   ...
				   test_outflows=False,outflow_test_niter=10,max_like_niter=10,
				   print_output=True):
	"""
	...
	testing outflows and fit for final initial paramters for emcee.
	...
	*NOTE*: As of v7.4.1, we have started using the scipy.optimize.basinhopping algorithm with SLSQP
	...
	get stuck in local minima, namely for the power_slope paramter.  While the 
	...
	if (all(comp in param_names for comp in ['na_oiii5007_core_amp','na_oiii5007_core_fwhm','na_oiii5007_core_voff',
		...
		if print_output:
			...
			print('\n Model 1: Hb/[OIII] Region with outflows, NO broad lines (excludes Ha/[NII]/[SII] region and outflows) \n')
			print('\n Using Basin-hopping algorithm to estimate paramters...\n ')
		cons = cons1
		start_time = time.time()
		...
		# 	 				 	   temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
		# 	 				 	   fit_type,output_model),\
		# 	 				 method='SLSQP', bounds = bounds, constraints=cons, options={'ftol':1.0e-6,'maxiter':2500,'disp': False})
		result = op.basinhopping(func = nll, x0 = params, \
			 				 minimizer_kwargs = {'args':(param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
			 				 	   temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
			 				 	   fit_type,output_model),\
			 				 'method':'SLSQP', 'bounds':bounds, 'constraints':cons},disp=False,niter_success=10)
	#
	...
	# Model 7: NO outflows, NO broad lines, Hb/[OIII]
		...
		result = op.basinhopping(func = nll, x0 = params, \
			 				 minimizer_kwargs = {'args':(param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
			 				 	   temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
			 				 	   fit_type,output_model),\
			 				 'method':'SLSQP', 'bounds':bounds},disp=False,niter_success=10)
	#
	...
	# Model 9: NO outflows, NO broad lines, Ha/[NII]/[SII]
		...
		# 	 				 	   temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
		# 	 				 	   fit_type,output_model),\
		# 	 				 method='SLSQP', bounds = bounds, options={'ftol':1.0e-6,'maxiter':2500,'disp': False})
		result = op.basinhopping(func = nll, x0 = params, \
			 				 minimizer_kwargs = {'args':(param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
			 				 	   temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
			 				 	   fit_type,output_model),\
	...
	if ((test_outflows==True) ):
		...
		comp_dict = fit_model(result['x'],param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
							  temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
							  fit_type,output_model)
			...
			if cons is not None:
				...
				resultmc = op.minimize(fun = nll, x0 = result['x'], \
			 				 		 args=(param_names,lam_gal,mcgal,mcnoise,gal_temp,na_feii_temp,br_feii_temp,
			 				 	   		   ...
			 				 	   		   fit_type,output_model),\
			 				 		 method='SLSQP', bounds = bounds, options={'maxiter':2500,'disp': False})
			# Append paramters to parameter dictionary
			for p,par in enumerate(param_names):	
				mcpars[par][n] = resultmc['x'][p]
			# Generate model components for each monte carlo iteration
			fit_type	 = 'outflow_test'
			output_model = False
			comp_dict = fit_model(resultmc['x'],param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
								  temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
								  fit_type,output_model)
...
def lnlike(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
	...
	if (fit_type=='final') and (output_model==False):
		model, blobs = fit_model(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
						  		 temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
						  		 fit_type,output_model)
...
def lnprob(params,param_names,bounds,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
	...
	fit_type	 = 'final'
	output_model = False
	ll, blobs 	 = lnlike(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
		   					temp_list,temp_fft,npad,fwhm_gal,velscale,npix,vsyst,run_dir,
		   					fit_type,output_model)
...
def fit_model(params,param_names,lam_gal,galaxy,noise,gal_temp,na_feii_temp,br_feii_temp,
	...
	# in the Ha/[NII]/[SII] region to be fit as free paramters.
...
def initialize_feii(lam_gal,velscale,fit_reg):
	"""
	Generate the broad and narrow FeII templates.
	"""
...
def feii_template(lam,rel_int,center,amp,sigma,voff,velscale,fit_reg):
	...
	for i in range(0,len(rel_int),1):
		if ((((fit_reg[0]+25.) < center[i] < (fit_reg[1]-25.))==True)):
			...
			continue
	# Sum the gaussians into a single template
...
def gaussian(x,center,amp,fwhm,voff,velscale):
	...
	"""
	x_pix = range(len(x))
	wave_interp = interp1d(x,x_pix,kind='linear',bounds_error=False,fill_value=(0,0))
...
def run_emcee(pos,ndim,nwalkers,run_dir,lnprob_args,init_params,param_names,
			  ...
			  print_output=True):
	"""
	Runs MCMC using emcee on all final paramters and checks for autocorrelation convergence 
	...
	dtype = [('fluxes',dict)]
	sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=lnprob_args,threads=threads,blobs_dtype=dtype)
			...
			try:
				...
				if all(elem in conv_type  for elem in param_names)==True:
					if (print_output):
						...
						if (print_output):
							print('\n One of more paramters in conv_type is not a valid parameter. Defaulting to median convergence type../.\n')
						conv_type='median'
...
def param_plots(param_dict,burn_in,run_dir,plot_param_hist=True,print_output=True):
	...
	free paramters from MCMC sample chains.
	"""
	# Create a histograms sub-folder
	if (plot_param_hist==True):
		if (os.path.exists(run_dir + 'histogram_plots')==False):
...
def write_param(param_dict,flux_dict,lum_dict,extra_dict,bounds,run_dir):
	"""
	Writes all measured paramters, fluxes, luminosities, and extra stuff 
...
def write_log(output_val,output_type,run_dir):
	...
	if (output_type=='outflow_test'):
		...
		# 			   resid_std_outflow,resid_std_outflow_err,
		# 			   ),'outflow_test',run_dir)
		(rdict,sigma,total_resid_noise,total_resid_noise_err,amp_metric,fwhm_metric,voff_metric,rov,rov_err,
		...
		logfile.write('\n{0:<30}{1:<30.4f}{2:<30.4f}'.format('ratio-of-variances',rov,rov_err))		
	...
	if (output_type=='outflow_test_pass'):
		outflow_test_pars = output_val
		logfile = open(run_dir+'log/log_file.txt','a')
		logfile.write('\n')
		logfile.write('\n### Tests for Outflows ###')
		logfile.write('\n---------------------------------------------------------------------')
		if (outflow_test_pars['amp_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('Outflow amplitude condition:'   , str(bool(outflow_test_pars['amp_test']['pass'])) ))
		if (outflow_test_pars['fwhm_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('Outflow FWHM condition:'	     , str(bool(outflow_test_pars['fwhm_test']['pass'])) ))
		if (outflow_test_pars['voff_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('Outflow VOFF condition:'	     , str(bool(outflow_test_pars['voff_test']['pass'])) ))
		if (outflow_test_pars['rov_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('ratio-of-variances condition:'  , str(bool(outflow_test_pars['rov_test']['pass'])) ))
		if (outflow_test_pars['bounds_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('Parameter bounds condition:'	 , str(bool(outflow_test_pars['bounds_test']['pass'])) ))
		logfile.write('\n---------------------------------------------------------------------')
